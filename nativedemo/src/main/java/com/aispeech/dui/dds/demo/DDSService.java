...
public class DDSService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if(intent.getAction().equals("start_inside")) {
            init();
        }
        else
        {
    ...
    private DDSInitListener mInitListener = new DDSInitListener() {
        ...
        public void onInitComplete(boolean isFull) {
            ...
            Intent i=new Intent();
            i.setAction("ddsdemo.intent.action.init_complete");
    ...
    private void init_outside() {
        DDS.getInstance().setDebugMode(2); //在调试时可以打开sdk调试日志，在发布版本时，请关闭
        Log.d(TAG, "Start DDS");
        DDS.getInstance().init(getApplicationContext(), createConfig2(), mInitListener, mAuthListener);
    }
    ...
    private DDSConfig createConfig2() {

        DDSConfig config = new DDSConfig();

        config.addConfig(DDSConfig.K_PRODUCT_ID, "278577182"); // 产品ID
        config.addConfig(DDSConfig.K_USER_ID, "15659528893");  // 用户ID
        ...
        config.addConfig(DDSConfig.K_API_KEY, "3291f82b730d3291f82b730d5c076674");  // 产品授权秘钥，服务端生成，用于产品授权
        config.addConfig(DDSConfig.K_DEVICE_ID, getDeviceId(getApplicationContext()));//填入唯一的deviceId
        // 基础配置项
//        config.addConfig(DDSConfig.K_PRODUCT_ID, "278576015"); // 产品ID
//        config.addConfig(DDSConfig.K_USER_ID, "13063196906");  // 用户ID
//        config.addConfig(DDSConfig.K_ALIAS_KEY, "test");   // 产品的发布分支
//        config.addConfig(DDSConfig.K_AUTH_TYPE, AuthType.PROFILE); //授权方式, 支持思必驰账号授权和profile文件授权
//        config.addConfig(DDSConfig.K_API_KEY, "dfad63e3ce28dfad63e3ce285bcd3b45");  // 产品授权秘钥，服务端生成，用于产品授权
//        config.addConfig(DDSConfig.K_DEVICE_ID, getDeviceId(getApplicationContext()));//填入唯一的deviceId
        //config.addConfig(DDSConfig.K_MIC_TYPE,5);//模组为家居双麦时添加此配置，其他模组在DUI控制台进行选择配置
...
//
//        // 录音配置项
        config.addConfig(DDSConfig.K_RECORDER_MODE, "external"); //录音机模式：external（使用外置录音机，需主动调用拾音接口）、internal（使用内置录音机，DDS自动录音）
